...
struct sup_pagetable_entry *
sup_pagetable_add_file (struct thread *thread, enum page_state state,
                        void *upage, struct file *file, off_t ofs,
                        uint32_t read_bytes, uint32_t zero_bytes, bool writable);
struct sup_pagetable_entry *
sup_pagetable_add_swap (void *upage, bool writable);

/* Functions to properly load the pages after a page fault occurs. */
bool sup_pagetable_load_all_zero (struct sup_pagetable_entry *entry);
bool sup_pagetable_load_file (struct sup_pagetable_entry *entry);
bool sup_pagetable_load_from_swap (struct sup_pagetable_entry *entry);

/* Helper functions. */
struct sup_pagetable_entry* sup_pagetable_entry_lookup(void * page);
